...
def recommend(input_data):
    ...
    for inpu in input_data:
        ...
        ads=[y for x in os.walk(ad_path) for y in glob(os.path.join(x[0], '*_v4.txt'))]
        ...
        try:
            ...
            test=test_path+str(input_split[1].replace("_v4.txt","_desc.txt"))
            with open(test,"r") as f:
                dat=f.readlines()
                f.close()
        data=[]
